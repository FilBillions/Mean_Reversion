...
# long and short logic ---- we want to close position if we are in the middle of the range
        ...
        self.df['Signal'] = np.where(self.df['Ratio'] < long, 1, np.nan)
        self.df['Signal'] = np.where(self.df['Ratio'] < long2, 1, np.nan)

        # Decrement position when crossing the short line
        self.df['Signal'] = np.where(self.df['Ratio'] > short, -1, self.df['Signal'])
        self.df['Signal'] = np.where(self.df['Ratio'] > short2, -1, self.df['Signal'])
...
# Iterate through the DataFrame row by row to update positions cumulatively
...
# Formatting the table
        self.df = round((self.df[['Day Count', 'Open', 'High', 'Low', 'Close', f'{self.ma1}-day SMA', 'Return', 'Cumulative Return', 'Mean Reversion Model Return', 'Cumulative Mean Reversion Model Return', 'Ratio', 'Position', 'Signal', 'Entry']]), 3)
...
# Plot buy signals (Entry == 2)
        plt.scatter(
            [x_values[i] for i in range(len(self.df.iloc[-model_days:])) if self.df.iloc[-model_days:].iloc[i]['Entry'] > 0],
            self.df.iloc[-model_days:]['Close'][self.df.iloc[-model_days:]['Entry'] > 0],
...
#print statements        
        ...
        print(f'count of buy signals: {len(self.df[self.df["Entry"] > 0])}')
        print(f'count of sell signals: {len(self.df[self.df["Entry"] < 0])}')
